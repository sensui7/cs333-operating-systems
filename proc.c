...
wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.pLists.ready; p && !havekids; p = p->next){
      if(p->parent == proc){
	havekids = 1;
      }
    }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Get head of ready list (round robin) and give it to CPU
...
zombiedump(void)
{
  ...
  while(current){
    ppid = (current->pid == 1) ? 1 : current->parent->pid;
    if(current->next == 0){
